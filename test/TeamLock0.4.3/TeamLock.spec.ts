...
describe('TeamLock Normal Case', () => {
  ...
  it('test contract in normal case', async () => {
    ...
    let result = await teamLock.query('getBenificiary', []);
    ...
    result = await teamLock.query('getBenificiary', []);
    ...
    expect(await teamLock.query('isOwner', [deployer.address])).to.be.deep.equal(['1']);
    expect(await teamLock.query('isOwner', [user1.address])).to.be.deep.equal(['1']);
    expect(await teamLock.query('isOwner', [user2.address])).to.be.deep.equal(['1']);
    expect(await teamLock.query('isOwner', [user3.address])).to.be.deep.equal(['0']);
    expect(await teamLock.query('isOwner', [user4.address])).to.be.deep.equal(['0']);
